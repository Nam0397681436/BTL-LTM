...
public class MatchDAO extends DAO {
    ...
    public void create(Match match) {
        ...
        try {
            ...
            try (PreparedStatement stmt = con.prepareStatement(sql1, PreparedStatement.RETURN_GENERATED_KEYS)) {
                ...
                stmt.setString(2, match.getHost().getPlayer().getPlayerId());
                stmt.executeUpdate();
                // Lấy id vừa insert
    ...
    //     String sqlMatch = "INSERT INTO player_matches (match_id, user_id, status, points, role) VALUES (?, ?, ?, ?, ?)";
    //     // String sqlUpdateUser = "UPDATE users SET total_points = total_points + ?, total_wins = total_wins + ? WHERE id = ?";
    //     // String sqlUpdateMatches = "UPDATE users SET total_matches = total_matches + 1 WHERE id = ?";
    ...
    //         //     for (Player player : match.getPlayers()) {
    //         //         updateUserStatsPvP(sqlUpdateUser, player.getPlayer().getId(), player.getStatus());
    //         //     }
    //         // }
