...
import client.ui.FindUserFRM;
import client.ui.GameWindowMultiplayerFRM;
import client.ui.HistoryFrame;
import client.ui.LoginFrame;
import client.ui.MainFrame;
import client.ui.MultiplayerRoomFRM;
...
public class ClientApp {
    ...
    public static void main(String[] args) {
        ...
        // Mở màn Đăng nhập. Khi AUTH_OK -> mở MainFrame (truyền kèm playerId &
        // nickname)
        ...
        try {
            tcp.connect(host, port, (line) -> SwingUtilities.invokeLater(() -> {
                for (java.awt.Window w : java.awt.Window.getWindows()) {
                    if (!w.isShowing())
                        continue;
                    if (w instanceof LoginFrame lf)
                        lf.handleLine(line);
                    if (w instanceof RegisterFrame rf)
                        rf.handleLine(line);
                    if (w instanceof MainFrame mf)
                        mf.handleLine(line);
                    if (w instanceof HistoryFrame hf)
                        hf.handleLine(line);
                    if (w instanceof FindUserFRM fu)
                        fu.handleLine(line);
                    if (w instanceof MultiplayerRoomFRM mr)
                        mr.handleLine(line);
                    if (w instanceof GameWindowMultiplayerFRM gf)
                        gf.handleLine(line);
                }
            }));
