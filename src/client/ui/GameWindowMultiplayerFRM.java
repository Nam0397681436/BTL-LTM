...
public class GameWindowMultiplayerFRM extends JFrame implements ActionListener {
    ...
    public GameWindowMultiplayerFRM(Match match, TcpClient tcp, MainFrame mainFrame) {
        this.match = match;
        this.tcp = tcp;
            ...
            try {
                ...
                switch (type) {
                    ...
                    case "START_NEXT_ROUND" -> {
                        ...
                        int round = msg.get("nextRound").getAsInt();
                        startMemoryGame(code, round);
                    }
                    ...
                    case "FINAL_RESULTS" -> {
                        Match finalMatch = Match.fromJson(msg.get("match").getAsString());
    ...
    public void startMemoryGame(String code, int round) {
        this.round = round;
        generatedString = code;
        gameLabel.setText(code);
        ...
        timeLeft = getTimeEnterInput(round);
    ...
    private static int getTimeEnterInput(int round) {
        if (round == 1)
            return 3;
        else if (round == 2)
            return 3;
    ...
    public void showRoundResult(Match match, List<RoundResult> roundResults) {
        if (resultTimer != null)
            resultTimer.stop();
        if (memorizeTimer != null)
            memorizeTimer.stop();
        ...
        for (RoundResult rr : roundResults) {
            resultText.append(rr.getUser().getUsername())
                    .append(" - Answer: ")
    ...
    private void showFinalResults(Match match) {
        if (resultTimer != null)
            resultTimer.stop();
        if (memorizeTimer != null)
            memorizeTimer.stop();
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            data[i][2] = p.getScore();
            data[i][3] = p.isWinner() ? "Winner" : "Loser";
        }
